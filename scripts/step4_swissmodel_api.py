...
def extract_quality_row(gene_id, result_json):
    """
    Pull GMQE/QMEAN/identity/coverage directly from JSON fields.
    Field names depend on the actual API response shape returned by
    SWISS-MODEL; this function should be adjusted to match the real
    response structure observed on first successful run, since the
    exact schema is not fully documented publicly. Fields default to
    None (not a placeholder number) if missing, so gaps are visible
    rather than silently filled with wrong values.
    """
    ...
    return {
        ...
        "qmean": best_model.get("qmean_norm") or best_model.get("qmean"),
        "identity": best_model.get("identity") or best_model.get("seqid"),
        "coverage": best_model.get("coverage"),
        "template": best_model.get("template"),
